 ...
 */
/* Arduino sdkconfig may set CONFIG_LVGL_PORT_AVOID_TEARING_MODE to 0; #ifdef is still true, so the old code kept tearing on.
 * Non-zero CONFIG wins; 0 or unset defaults to mode 3 (RGB direct mode). */
#if defined(CONFIG_LVGL_PORT_AVOID_TEARING_MODE)
#if CONFIG_LVGL_PORT_AVOID_TEARING_MODE > 0
