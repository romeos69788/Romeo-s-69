...
#if defined(ROMEOS_LINK_ESPNOW) && ROMEOS_LINK_ESPNOW
...
uint8_t g_telemetry_streak = 0;
uint8_t g_mb_mac[6]{};
bool g_have_mb_mac = false;

constexpr uint32_t k_rx_stale_ms = 10000;
constexpr uint8_t k_telemetry_streak_need = 2;
constexpr uint32_t k_tx_interval_ms = 400;

void remember_mb_peer(const uint8_t mac[6])
{
    if (!mac) {
        return;
    }
    if (g_have_mb_mac && std::memcmp(g_mb_mac, mac, 6) == 0) {
        return;
    }
    std::memcpy(g_mb_mac, mac, 6);
    g_have_mb_mac = true;
    romeos_espnow_add_peer(mac);
    if (romeos_espnow_save_peer_mac("mb_mac", mac)) {
        Serial.printf("[espnow] paired motherboard %02X:%02X:%02X:%02X:%02X:%02X\n",
                      mac[0],
                      mac[1],
                      mac[2],
                      mac[3],
...
void handle_mb_to_display(const romeos_mb_to_display_v1_t *pkt, const uint8_t src_mac[6])
{
    if (!pkt || !src_mac) {
        return;
    }
    if (pkt->magic != ROMEOS_LINK_MAGIC ||
        (pkt->version != 1u && pkt->version != static_cast<uint8_t>(ROMEOS_LINK_VER))) {
        return;
    }
    remember_mb_peer(src_mac);
    static bool s_logged_link;
    if (!s_logged_link) {
        s_logged_link = true;
        Serial.println(F("[espnow] link UP — receiving MB telemetry"));
    }
...
void on_disp_espnow_rx(const uint8_t *data, size_t len, const uint8_t src_mac[6])
{
    if (!data || len < static_cast<size_t>(sizeof(romeos_mb_to_display_v1_t))) {
        return;
    }
    romeos_mb_to_display_v1_t pkt{};
    std::memcpy(&pkt, data, sizeof(pkt));
    handle_mb_to_display(&pkt, src_mac);
}
                              ...
                              romeos_get_room_c_x10_fn get_room_c_x10)
{
    ...
    Serial.println(F("[romeos_link] ESP-NOW = μόνιμο link MB↔οθόνη (ανεξάρτητο από home Wi‑Fi)"));
    if (!romeos_espnow_begin("disp")) {
        Serial.println(F("[espnow] FATAL init failed"));
        return;
    }
    romeos_espnow_set_rx_cb(on_disp_espnow_rx);
    uint8_t saved[6]{};
    if (romeos_espnow_load_peer_mac("mb_mac", saved)) {
        remember_mb_peer(saved);
        Serial.println(F("[espnow] loaded motherboard peer from NVS"));
    }
}
...
{
    const uint32_t now = millis();
    return g_last_rx_ms != 0u && (now - g_last_rx_ms) < k_rx_stale_ms;
}

bool romeos_display_link_home_wifi_assoc(void)
{
#if defined(ROMEOS_WIFI_HOME_ONLY) && ROMEOS_WIFI_HOME_ONLY
    return romeos_display_wifi_home_assoc();
#else
    return false;
#endif
}
...
void romeos_display_link_poll(void)
{
    ...
    if (g_last_rx_ms != 0 && (now - g_last_rx_ms) >= k_rx_stale_ms) {
        g_telemetry_streak = 0;
    }
    ...
    if (g_have_mb_mac) {
        romeos_espnow_send(g_mb_mac, &out, sizeof(out));
    } else {
        romeos_espnow_send_broadcast(&out, sizeof(out));
    }
}
...
{
#if defined(ROMEOS_WIFI_HOME_ONLY) && ROMEOS_WIFI_HOME_ONLY
    romeos_display_wifi_home_on_nvs_saved();
#endif
}

#else /* !ROMEOS_LINK_ESPNOW */
...
IPAddress mb_tx_destination()
{
    ...
    if (connected_to_home_ssid()) {
        ...
        if (loc[0] != 0) {
            return IPAddress(static_cast<uint8_t>(loc[0] | (~mask[0] & 0xFF)),
                             static_cast<uint8_t>(loc[1] | (~mask[1] & 0xFF)),
                             static_cast<uint8_t>(loc[2] | (~mask[2] & 0xFF)),
                             static_cast<uint8_t>(loc[3] | (~mask[3] & 0xFF)));
        }
        if (!g_warned_lan_ip) {
            g_warned_lan_ip = true;
            Serial.println(F("[romeos_link] home Wi‑Fi OK but no LAN IP yet — waiting for DHCP"));
        }
        return IPAddress(0, 0, 0, 0);
    }
