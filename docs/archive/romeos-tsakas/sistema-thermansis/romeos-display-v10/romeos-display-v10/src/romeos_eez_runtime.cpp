...
#ifndef ROMEOS_DISPLAY_MIC_ENABLE
#define ROMEOS_DISPLAY_MIC_ENABLE 0
#endif
...
static bool s_room_sensor_present = false;
...
static void room_sensor_ui_apply_missing_unlocked(void)
{
    label_set_if_changed(objects.obj9, "--");
    label_set_if_changed(objects.obj32, "--");
    humidity_label_set(objects.obj15, "--");
    humidity_label_set(objects.obj38, "--");
}

static void room_sensor_ui_apply_missing(void)
{
    if (!lvgl_port_lock(80)) {
        return;
    }
    room_sensor_ui_apply_missing_unlocked();
    lvgl_port_unlock();
}
...
/** Εξωτερική θερμοκρασία (UDP x10): μόνο ακέραιοι βαθμοί — 32, όχι 32.2. */
static void format_outdoor_temp_x10(int16_t t_x10, char *out, size_t len)
{
    if (t_x10 == INT16_MIN) {
        std::snprintf(out, len, "--");
        return;
    }
    const int deg = static_cast<int>((t_x10 >= 0 ? t_x10 + 5 : t_x10 - 5) / 10);
    std::snprintf(out, len, "%d", deg);
}
...
static bool mb_outdoor_humidity_valid(const romeos_mb_to_display_v1_t &pkt)
{
    if ((pkt.mb_proto_reserved & 0x04u) == 0u) {
        return false;
    }
    const int16_t rh = pkt.outdoor_rh_x10;
    return rh >= 0 && rh <= 1000;
}
...
{
    const uint32_t now = millis();
    if (now - s_last_sensor_ms < 1000u) {
        return;
    }
    s_last_sensor_ms = now;

    if (!s_room_sensor_present ||
        std::strcmp(romeos_room_sht_touch_bus_family_name(), "none") == 0) {
        room_sensor_ui_apply_missing();
        return;
    }

    float t_c = NAN, rh = NAN;
    if (!romeos_room_sht_touch_bus_read_both(&t_c, &rh) || !std::isfinite(t_c)) {
        room_sensor_ui_apply_missing();
        return;
    }
...
static void outdoor_sensor_ui_update(bool force)
{
    static uint32_t s_last_outdoor_ms = 0;
    const uint32_t now = millis();
    if (!force && s_last_outdoor_ms != 0u && (now - s_last_outdoor_ms) < 1000u) {
        return;
    }
    ...
    if (romeos_display_link_telemetry_valid() && s_mb_pkt_fresh &&
        ...
        format_outdoor_temp_x10(s_mb_pkt.outdoor_c_x10, tbuf, sizeof(tbuf));
    } else {
        std::snprintf(tbuf, sizeof(tbuf), "--");
    }

    if (romeos_display_link_telemetry_valid() && s_mb_pkt_fresh &&
        mb_outdoor_humidity_valid(s_mb_pkt)) {
        const int rhi = static_cast<int>((s_mb_pkt.outdoor_rh_x10 + 5) / 10);
        std::snprintf(rhbuf, sizeof(rhbuf), "%d", rhi);
    } else {
        std::snprintf(rhbuf, sizeof(rhbuf), "--");
    }
    ...
    humidity_label_set(objects.obj14, rhbuf);
    humidity_label_set(objects.obj37, rhbuf);
    lvgl_port_unlock();
}

static void outdoor_sensor_ui_poll(void)
{
    outdoor_sensor_ui_update(false);
}
...
static void voice_cmd_handler(const char *cmd)
{
    if (cmd == nullptr) {
        return;
    }

    ScreensEnum id = SCREEN_ID_PRYMARY;
    if (std::strcmp(cmd, "menu") == 0) {
        id = SCREEN_ID_MENU;
    } else if (std::strcmp(cmd, "heat") == 0) {
        id = SCREEN_ID_HEAT;
    } else {
        return;
    }

    Serial.printf("[eez] voice navigate -> screen %d\n", static_cast<int>(id));
    if (lvgl_port_lock(80)) {
        loadScreen(id);
        lvgl_port_unlock();
    }
...
static void mic_on_click(lv_event_t *e)
{
    ...
    display_mic_set_listening(s_mic_active);
#if !ROMEOS_DISPLAY_MIC_ENABLE
    if (s_mic_active) {
        s_mic_active = false;
        mic_set_visual(false);
        Serial.println(F("[eez] MIC disabled in build (ROMEOS_DISPLAY_MIC_ENABLE=0)"));
    }
#endif
}
...
static int16_t link_get_setpoint_x10()
{
    int v = static_cast<int>(std::lround(s_setpoint * 10.0f));
    if (v < 50) {
        v = 50;
    }
    if (v > 350) {
...
static int16_t link_get_room_c_x10()
{
    return ROMEOS_ROOM_FROM_DISPLAY_INVALID;
}

void init_display_link_services(void)
{
    romeos_display_link_init(on_mb_telemetry, link_get_setpoint_x10, link_get_room_c_x10);
}
...
{
    delay(120);
    s_room_sensor_present = romeos_room_sht_touch_bus_begin_auto();
    Serial.printf("[eez] room I2C sensor: %s (%s)\n",
                  s_room_sensor_present ? "OK" : "NOT FOUND",
...
#if defined(ROMEOS_LINK_ESPNOW) && ROMEOS_LINK_ESPNOW && defined(ROMEOS_WIFI_HOME_ONLY) && \
    ROMEOS_WIFI_HOME_ONLY
    romeos_display_wifi_home_begin();
#endif
    init_display_link_services();

#if ROMEOS_DISPLAY_MIC_ENABLE
    display_mic_set_voice_handler(voice_cmd_handler);
    display_mic_init();
    Serial.println(F("[eez] mic enabled"));
#else
    Serial.println(F("[eez] mic OFF (paused) — no I2S at boot"));
#endif
