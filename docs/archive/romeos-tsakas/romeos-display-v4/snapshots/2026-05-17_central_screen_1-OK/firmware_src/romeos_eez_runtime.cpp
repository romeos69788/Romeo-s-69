...
static void wifi_ui_apply(bool home)
{
    ...
    apply_wifi_status_label(objects.obj10, home);
    apply_wifi_status_label(objects.obj26, home);
    lvgl_port_unlock();
}
...
static void clock_apply_tm(const struct tm *tm)
{
    ...
    label_set_if_changed(objects.obj11, hbuf);
    label_set_if_changed(objects.obj13, mbuf);
    if (objects.obj14 != nullptr && lv_obj_is_valid(objects.obj14)) {
        ...
        label_set_if_changed(objects.obj14, ap);
    }

    if (objects.obj2 != nullptr && tm->tm_wday >= 0 && tm->tm_wday <= 6) {
        label_set_if_changed(objects.obj2, k_wdays[tm->tm_wday]);
    }
    if (objects.obj3 != nullptr) {
        char dbuf[8];
        std::snprintf(dbuf, sizeof(dbuf), "%d", tm->tm_mday);
        label_set_if_changed(objects.obj3, dbuf);
    }
    if (objects.obj25 != nullptr && tm->tm_mon >= 0 && tm->tm_mon <= 11) {
        label_set_if_changed(objects.obj25, k_months[tm->tm_mon]);
    }
...
static void colon_blink_tick(uint32_t now)
{
    ...
    if (objects.obj12 != nullptr && lv_obj_is_valid(objects.obj12)) {
        if (s_colon_visible) {
...
static void sensor_poll(void)
{
    ...
    label_set_if_changed(objects.obj15, tbuf);
    label_set_if_changed(objects.obj18, rhbuf);
    lvgl_port_unlock();
}
...
static void alarm_set_visual(bool active)
{
    if (objects.obj7 != nullptr && lv_obj_is_valid(objects.obj7)) {
        const lv_color_t c = lv_color_hex(active ? 0xFF3B30 : 0xFFFFFF);
        lv_obj_set_style_text_color(objects.obj7, c, LV_PART_MAIN | LV_STATE_DEFAULT);
    }
}
...
void romeos_eez_runtime_after_ui_init(void)
{
    ...
    alarm_bind_btn(objects.alarm_button);
    alarm_bind_btn(objects.alarm);
    alarm_set_visual(false);
    apply_wifi_status_label(objects.obj10, false);
    apply_wifi_status_label(objects.obj26, false);
}
