...
static bool s_alt_bus_started = false;
static bool s_using_alt_wire1 = false;
...
void romeos_room_temp_sensor_after_board_begin()
{
    ...
    if (!g_room_sensor_ready) {
        // Fallback: some boards route the room sensor to dedicated pins instead of shared touch bus.
        constexpr int alt_sda = 43;
        constexpr int alt_scl = 44;
#if defined(CONFIG_IDF_TARGET_ESP32S3)
        if (uart_is_driver_installed(UART_NUM_0)) {
            (void)uart_driver_delete(UART_NUM_0);
        }
#endif
        s_sht_wire.begin(alt_sda, alt_scl);
        s_sht_wire.setClock(100000);
        s_alt_bus_started = true;
        g_room_sensor_ready = s_room_adafruit.begin(0x44);
        if (!g_room_sensor_ready) {
            g_room_sensor_ready = s_room_adafruit.begin(0x45);
        }
        s_using_alt_wire1 = g_room_sensor_ready;
        Serial.printf("[room_temp] fallback Wire1 SDA=%d SCL=%d -> %s\n",
                      alt_sda,
                      alt_scl,
                      g_room_sensor_ready ? "ready" : "NOT FOUND");
    }
...
void romeos_room_temp_sensor_poll_in_app_loop()
{
    ...
    if (!g_room_sensor_ready) {
        static uint32_t s_last_not_ready_log_ms = 0;
        if (now - s_last_not_ready_log_ms >= 5000u) {
            s_last_not_ready_log_ms = now;
#if ROMEOS_ROOM_I2C_SHARED_TOUCH_BUS
            Serial.println("[room_temp] sensor NOT FOUND yet on shared bus");
#else
            Serial.println("[room_temp] sensor NOT FOUND yet on Wire1");
#endif
            Serial.flush();
        }
#if ROMEOS_ROOM_I2C_SHARED_TOUCH_BUS
        ...
        if (now - s_last_late_detect_ms >= 45000u) {
            ...
            if (romeos_room_sht_touch_bus_begin_auto()) {
                ...
                if (g_room_sensor_ready) {
                    s_using_alt_wire1 = true;
                    Serial.printf("[room_temp] late fallback Wire1 detect OK SDA=%d SCL=%d\n", alt_sda, alt_scl);
                    Serial.flush();
                }
            }
        }
    ...
    bool ok = false;
#if ROMEOS_ROOM_I2C_SHARED_TOUCH_BUS
    if (s_using_alt_wire1) {
        ok = s_room_adafruit.readBoth(&raw_tc, &rh);
    } else {
        ok = read_both_shared(&raw_tc, &rh);
    }
#else
    ok = read_both_shared(&raw_tc, &rh);
#endif
    const float tc = ok ? raw_tc : NAN;
