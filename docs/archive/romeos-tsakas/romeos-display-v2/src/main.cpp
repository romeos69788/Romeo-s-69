...
#include "squareline/screens/ui_THERMOSTAT_1.h"
#endif
...
enum class ClockEdit : uint8_t { None, Hours, Minutes, AmPm, Weekday, Day, Month, Year };
...
static uint8_t s_date_weekday = 1; /* 0=Sunday .. 6=Saturday */
static uint8_t s_date_day = 15;
static uint8_t s_date_month = 10;
static uint16_t s_date_year = 2026;
...
static void squareline_save_clock_to_nvs()
{
    ...
    s_clock_prefs.putUChar("wd", s_date_weekday);
    s_clock_prefs.putUChar("d", s_date_day);
    s_clock_prefs.putUChar("mo", s_date_month);
    s_clock_prefs.putUShort("y", s_date_year);
}
...
static void squareline_load_clock_from_nvs()
{
    ...
    const uint8_t wd = p.getUChar("wd", 1);
    const uint8_t d = p.getUChar("d", 15);
    const uint8_t mo = p.getUChar("mo", 10);
    const uint16_t y = p.getUShort("y", 2026);
    p.end();
    if (h < 24 && m < 60 && s < 60) {
        s_clock_h24 = h;
        s_clock_min = m;
...
{
    if (ui_Label15) {
        lv_obj_set_style_text_opa(ui_Label15, s_clock_blink_on ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label12) {
        const bool show = (s_clock_edit != ClockEdit::Hours) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label12, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label16) {
        const bool show = (s_clock_edit != ClockEdit::Minutes) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label16, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label17) {
        const bool show = (s_clock_edit != ClockEdit::AmPm) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label17, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label4) {
        const bool show = (s_clock_edit != ClockEdit::Weekday) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label4, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label6) {
        const bool show = (s_clock_edit != ClockEdit::Day) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label6, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label9) {
        const bool show = (s_clock_edit != ClockEdit::Month) || s_clock_blink_on;
        lv_obj_set_style_text_opa(ui_Label9, show ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
    }
    if (ui_Label11) {
        const bool show = (s_clock_edit != ClockEdit::Year) || s_clock_blink_on;
...
static void squareline_refresh_clock()
{
    ...
    lv_label_set_text(ui_Label16, buf);

    if (ui_Label17) {
        lv_label_set_text(ui_Label17, (s_clock_h24 >= 12) ? "PM" : "AM");
    }

    static const char *const k_weekdays[] = {"SUNDAY", "MONDAY", "TUESDAY", "WEDNESDAY",
                                              "THURSDAY", "FRIDAY", "SATURDAY"};
    if (ui_Label4) {
        lv_label_set_text(ui_Label4, k_weekdays[s_date_weekday % 7]);
    }
    if (ui_Label6) {
        std::snprintf(buf, sizeof(buf), "%02u", static_cast<unsigned>(s_date_day));
        lv_label_set_text(ui_Label6, buf);
    }
    if (ui_Label9) {
        std::snprintf(buf, sizeof(buf), "%02u", static_cast<unsigned>(s_date_month));
        lv_label_set_text(ui_Label9, buf);
    }
    if (ui_Label11) {
        std::snprintf(buf, sizeof(buf), "%04u", static_cast<unsigned>(s_date_year));
...
static void squareline_on_clock_weekday_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
    s_clock_edit = (s_clock_edit == ClockEdit::Weekday) ? ClockEdit::None : ClockEdit::Weekday;
    s_clock_blink_on = true;
    squareline_apply_clock_visual_state();
}

static void squareline_on_clock_day_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
    s_clock_edit = (s_clock_edit == ClockEdit::Day) ? ClockEdit::None : ClockEdit::Day;
    s_clock_blink_on = true;
    squareline_apply_clock_visual_state();
}

static void squareline_on_clock_month_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
    s_clock_edit = (s_clock_edit == ClockEdit::Month) ? ClockEdit::None : ClockEdit::Month;
    s_clock_blink_on = true;
    squareline_apply_clock_visual_state();
}

static void squareline_on_clock_year_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
    s_clock_edit = (s_clock_edit == ClockEdit::Year) ? ClockEdit::None : ClockEdit::Year;
    s_clock_blink_on = true;
    squareline_apply_clock_visual_state();
}

static uint8_t squareline_days_in_month(uint16_t y, uint8_t m)
{
    static const uint8_t k_dim[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    uint8_t d = k_dim[(m - 1) % 12];
...
static void squareline_clock_adjust(bool increment)
{
    ...
    } else if (s_clock_edit == ClockEdit::Weekday) {
        s_date_weekday = static_cast<uint8_t>((s_date_weekday + (increment ? 1 : 6)) % 7);
    } else if (s_clock_edit == ClockEdit::Day) {
        const uint8_t max_day = squareline_days_in_month(s_date_year, s_date_month);
        if (increment) {
            s_date_day = (s_date_day >= max_day) ? 1 : static_cast<uint8_t>(s_date_day + 1);
        } else {
            s_date_day = (s_date_day <= 1) ? max_day : static_cast<uint8_t>(s_date_day - 1);
        }
    } else if (s_clock_edit == ClockEdit::Month) {
        if (increment) {
            s_date_month = (s_date_month >= 12) ? 1 : static_cast<uint8_t>(s_date_month + 1);
        } else {
            s_date_month = (s_date_month <= 1) ? 12 : static_cast<uint8_t>(s_date_month - 1);
        }
        const uint8_t max_day = squareline_days_in_month(s_date_year, s_date_month);
        if (s_date_day > max_day) {
            s_date_day = max_day;
        }
    } else if (s_clock_edit == ClockEdit::Year) {
        if (increment) {
            s_date_year = static_cast<uint16_t>((s_date_year >= 2099) ? 2000 : s_date_year + 1);
        } else {
            s_date_year = static_cast<uint16_t>((s_date_year <= 2000) ? 2099 : s_date_year - 1);
        }
        const uint8_t max_day = squareline_days_in_month(s_date_year, s_date_month);
        if (s_date_day > max_day) {
            s_date_day = max_day;
...
static void squareline_on_lights_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
...
static void squareline_on_heat_pump_clicked(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) {
        return;
    }
...
static void squareline_bind_romeos_controls()
{
    ...
    if (ui_Label17) {
        ...
        lv_obj_set_ext_click_area(ui_Label17, 12);
        lv_obj_add_event_cb(ui_Label17, squareline_on_clock_ampm_clicked, LV_EVENT_CLICKED, nullptr);
    }

    if (ui_Label12) {
        lv_obj_add_flag(ui_Label12, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Label12, 12);
        lv_obj_add_event_cb(ui_Label12, squareline_on_clock_hours_clicked, LV_EVENT_CLICKED, nullptr);
    }
    if (ui_Label16) {
        lv_obj_add_flag(ui_Label16, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Label16, 12);
        lv_obj_add_event_cb(ui_Label16, squareline_on_clock_minutes_clicked, LV_EVENT_CLICKED, nullptr);
    }
    ...
    if (ui_Button3) {
        ...
        lv_obj_set_ext_click_area(ui_Button3, 20);
        lv_obj_add_event_cb(ui_Button3, squareline_on_lights_clicked, LV_EVENT_CLICKED, nullptr);
    }
    if (ui_Button4) {
        lv_obj_add_flag(ui_Button4, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Button4, 20);
        lv_obj_add_event_cb(ui_Button4, squareline_on_heat_pump_clicked, LV_EVENT_CLICKED, nullptr);
    }

    if (ui_Button11) {
        lv_obj_add_flag(ui_Button11, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Button11, 12);
        lv_obj_add_event_cb(ui_Button11, squareline_on_clock_weekday_clicked, LV_EVENT_CLICKED, nullptr);
    }
    if (ui_Button12) {
        lv_obj_add_flag(ui_Button12, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Button12, 10);
        lv_obj_add_event_cb(ui_Button12, squareline_on_clock_day_clicked, LV_EVENT_CLICKED, nullptr);
    }
    if (ui_Button13) {
        lv_obj_add_flag(ui_Button13, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Button13, 10);
        lv_obj_add_event_cb(ui_Button13, squareline_on_clock_month_clicked, LV_EVENT_CLICKED, nullptr);
    }
    if (ui_Button14) {
        lv_obj_add_flag(ui_Button14, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_ext_click_area(ui_Button14, 10);
        lv_obj_add_event_cb(ui_Button14, squareline_on_clock_year_clicked, LV_EVENT_CLICKED, nullptr);
    }
...
static void ntp_apply_wall_time(const struct tm *tm)
{
    ...
    s_date_weekday = static_cast<uint8_t>(tm->tm_wday % 7);
    s_date_day = static_cast<uint8_t>(std::clamp(tm->tm_mday, 1, 31));
    s_date_month = static_cast<uint8_t>(std::clamp(tm->tm_mon + 1, 1, 12));
    s_date_year = static_cast<uint16_t>(std::clamp(tm->tm_year + 1900, 2000, 2099));
    squareline_clock_rebase_to_current_hms();
    if (lvgl_port_lock(50)) {
        squareline_refresh_clock();
        lvgl_port_unlock();
