...
export function App() {
  ...
  /** Αποφυγή “αναπήδησης” draft setpoint όταν έρχεται παλιό retained/out-of-order state αμέσως μετά από publish. */
  const lastLocalSetpointPublishMsRef = useRef<number>(0);
  const lastLocalSetpointX10Ref = useRef<number | null>(null);
  ...
  const publishCommand = useCallback(
    (payload: Record<string, number>, successHint: string) => {
      ...
      setErr(null);
      if (typeof payload.setpoint_c_x10 === "number") {
        lastLocalSetpointPublishMsRef.current = Date.now();
        lastLocalSetpointX10Ref.current = payload.setpoint_c_x10;
      }
    ...
    c.on("message", (topic, payload) => {
      ...
      try {
        ...
        if (typeof j.setpoint_c_x10 === "number") {
          const now = Date.now();
          const withinHoldMs = 8000;
          const recent = now - lastLocalSetpointPublishMsRef.current <= withinHoldMs;
          const expected = lastLocalSetpointX10Ref.current;
          if (
            recent &&
            expected !== null &&
            j.setpoint_c_x10 !== expected
          ) {
            // Κράτα το UI στο τοπικό setpoint μέχρι να “πιάσει” η μητρική στο state.
          } else {
  ...
  const heaterOnFromTelemetry = useMemo(() => {
    ...
    if (typeof state.relay_k4 === "string") {
      const n = parseInt(state.relay_k4, 10);
      if (!Number.isNaN(n)) {
        return n !== 0;
      }
    }
  ...
  const relayPills = useMemo(() => {
    ...
    return (
      <div className="row">
        {items.map(({ k, v }) => {
          const on =
            k === "K4" && heaterOnFromTelemetry !== null
              ? heaterOnFromTelemetry
              : Boolean(v);
          const pillClass =
            k === "K4" && !on ? "pill k4-off-state" : `pill ${on ? "on" : "off"}`;
          return (
      ...
      <div className="card stack">
        ...
        <div className="row boiler-row">
          ...
          <button
            ...
            className={[
              "secondary",
              heaterOnFromTelemetry === false ? "preset-on" : "",
            ]
              .filter(Boolean)
              .join(" ")}
            style={
              heaterOnFromTelemetry === false
                ? { backgroundColor: "#1d4ed8", color: "#fff" }
                : undefined
            }
