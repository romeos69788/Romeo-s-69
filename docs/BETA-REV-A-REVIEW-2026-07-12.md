...
| **CN9** 2P | ✓ | — | έλεγξε net (5V in;) |
| Silk REL1–REL4 | — | ✓ | βλ. πίνακα §4 |
| **CN_ALPHA / UART** προς Alpha | ✗ | ✗ | 🔴 **λείπει** |
...
| Στο Beta τώρα | Απόφαση project | Ενέργεια |
|---------------|-----------------|----------|
| **CN1 AC FLOW** | Όχι flow sensors (CN7/8 αφαιρέθηκαν Alpha) | **Διέγραψε** CN1 flow |
| **CN4 WATER FLOW** | Ίδιο | **Διέγραψε** CN4 flow |
| **CN2/CN3 DS18** | Όλα DS18 → **Alpha CN3/4/10** GPIO4 | **Διέγραψε** — μην διπλομετράς |
| **CN1** όνομα | Alpha **CN1 = ROOM-NTC** | Μην μπερδεύεις designator — Beta χωρίς CN1 ή άλλο όνομα |
| **DEFROST** | **Alpha CN_DEFROST** GPIO14 | ✓ όχι στη Beta |
| **Λογική/MQTT** | **Alpha ESP32** | Beta = εκτελεστής ρελέ (+ optional local ESP32 slave) |
...
| # | Item | Προτεραιότητα |
|---|------|----------------|
| 1 | **CN_ALPHA** 4P UART (GND · RX · TX · NC) | 🔴 κρίσιμο |
| 2 | **Αφαίρεση** CN1 flow · CN4 flow · CN2/CN3 DS18 | 🔴 κρίσιμο |
...
**Όχι fab ακόμα** — χρειάζεται:

1. **Καθάρισμα sensors** (flow + DS18) — σύμφωνα με Alpha rev A locked  
2. **CN_ALPHA UART** connector  
3. **Κλείδωμα πίνακα K1–K8** + GPIO map  
4. DRC clean  

Μετά → ίδια ροή με Alpha (Gerber preview → order).
